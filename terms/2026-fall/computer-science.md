---
layout: default
title: Computer Science — Agenda
---

# Computer Science — Agenda

[← Back to all classes](../../index.html)

## Next Class
**Monday, Sept 21** — *Plan not yet posted. Check back for what's next.*

---

<details markdown="1" open>
<summary><strong>Week 4 (Sept 14–18)</strong></summary>

### Friday, Sept 18
- Unit 1 quiz
- CertMaster work time (Module 2)

### Thursday, Sept 17
- Gimkit review for the quiz
- CertMaster work time (Module 2)
- More Gimkit review to close out class

### Wednesday, Sept 16
- CertMaster work time (Module 2) — the "Human Internet" opener got cut for time today, carries forward to whenever it next fits

### Tuesday, Sept 15
- Opener: "What is the Internet?" (Code.org slides)
- CertMaster work time — most students now in Module 2

### Monday, Sept 14
- CertMaster work time

</details>

<details markdown="1">
<summary><strong>Week 3 (Sept 8–11)</strong></summary>

### Friday, Sept 11
- Opener: CompTIA's Job Data Exploration tool — "Which job stands out to you?"
- CertMaster work time (Module 1/2)
- Exit journal: "What's a tech job you'd actually consider, and one thing you'd need to learn first?"

### Thursday, Sept 10
- Opener: "What does GIGO stand for?" (garbage in, garbage out)
- Journal: "When has bad input given you a bad result?"
- Framework: input → processing → storage → output
- CertMaster work time
- Exit journal: pick something you did on a device today and break it into all four stages — which stage was hardest to identify, and why?

### Wednesday, Sept 9
- Opener: name a job that exists today that didn't 20 years ago, and one that's disappeared because of technology
- Vocab hunt (DHCP, IP address, malware) while working through CertMaster
- CertMaster work time (Module 1.1)
- Discussion: what does digital citizenship mean?
- Exit journal: "A good digital citizen \_\_\_, but a bad one \_\_\_."

### Tuesday, Sept 8
- CertMaster Learn access confirmed — logged in and started Module 1

</details>

<details markdown="1">
<summary><strong>Week 2 (Aug 31–Sept 4)</strong></summary>

### Friday, Sept 4
- Completed two code.org AP CSP lessons (Lessons 10 and 11)

### Thursday, Sept 3
- code.org AP CSP: Lesson 9 — Lossless Compression
- Brief overview of what the AP CSP exam involves

### Wednesday, Sept 2
- code.org AP CSP: Lesson 8

### Tuesday, Sept 1
- code.org AP CSP: Lesson 7

### Monday, Aug 31
- code.org AP CSP: Lesson 6

</details>

<details markdown="1">
<summary><strong>Week 1 (Aug 24–28)</strong></summary>

### Friday, Aug 28
- code.org AP CSP: Lesson 5 — overflow/rounding

### Thursday, Aug 27
- code.org AP CSP: Lesson 4 — Binary Numbers, "Flippy Do" activity

### Wednesday, Aug 26
- code.org AP CSP: Lesson 3 — "Circle, Square"

### Tuesday, Aug 25
- Got set up on Google Classroom
- code.org AP CSP: Lesson 2 — Digital Information (binary numbers, data compression)

### Monday, Aug 24
- Class logistics
- 10-question practice quiz over Tech+ certification exam content

</details>
