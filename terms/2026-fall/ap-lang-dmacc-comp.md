---
layout: default
title: AP Lang / DMACC Comp — Agenda
---

# AP Lang / DMACC Comp — Agenda

[← Back to all classes](../../index.html)

## Next Class
**Wednesday, Sept 23** — *Plan, not yet confirmed. Check back after class for what actually happened.*

- Precision Wednesday: Paraphrase vs. Summary vs. Quotation
- *They Say, I Say* Chapters 1–3 project, part 2: find one example of each chapter's move from a class text or your own writing, then build gallery-walk slides
- Annotate Marriott's essay for the same Ch. 1–3 moves (if done with TSIS work)

---

<details markdown="1" open>
<summary><strong>Week 5 (Sept 21–25)</strong></summary>

### Tuesday, Sept 22
- Grammar
- *They Say, I Say* Chapters 1–3 group project: filled in a Moves Chart (why each move matters, a tell-tale signal, a common error)
- Marriott handed back for students to start finding moves in it independently

### Monday, Sept 21
- In-class writing: testing your This I Believe claim against Marriott's essay

</details>

<details markdown="1">
<summary><strong>Week 4 (Sept 14–18)</strong></summary>

### Friday, Sept 18
- Fallacy Review practice set (Google Classroom warmup)
- Fallacy Fridays: Genetic Fallacy and Appeal to Authority
- Finished This I Believe — turned in electronically, plus the peer-review form and a printed copy
- Remaining time: Marriott reading (writing task due Monday, Sept 21)

### Thursday, Sept 17
- Grammar: Semicolons Quick Reteach + Active Voice 2.1
- APA formatting (self-directed): made a new doc tab and formatted it using the [Purdue OWL APA Sample Paper](https://owl.purdue.edu/owl/research_and_citation/apa_style/apa_formatting_and_style_guide/apa_sample_paper.html) page — no instructions beyond "APA format." Not graded on accuracy.
- Printed the formatted draft, peer review with a partner
- Remaining time: Marriott extension task

### Wednesday, Sept 16
- Peer review: partner reads your draft, reviews your PAT self-reflection, and agrees/disagrees with your self-assessment (in person or with anyone else you ask, from either section)
- Extension task for early finishers: reading/analysis on James Marriott's "The Dawn of the Post-Literate Society" — due Monday, Sept 21
- Precision Wednesdays: Abstract vs. Concrete notes

### Tuesday, Sept 15
- Grammar Notes: Dashes & Active Voice
- Continued This I Believe drafting, with one-on-one conferences for students who'd finished their draft and self-check
- Everyone did the "PAT the SPACECAT" self-check by end of class, regardless of draft status — peer review moved to Wednesday

### Monday, Sept 14
- Alchemy: "Just Ten Images" (Ash Parsons) — reading + journal
- This I Believe drafting time
- "PAT the SPACECAT" self-check (Purpose, Audience, Tone) — added as a new tab in your own drafting doc today instead of a partner swap

</details>

<details markdown="1">
<summary><strong>Week 3 (Sept 8–11)</strong></summary>

### Friday, Sept 11
- Grammar quiz (Internal Punctuation Quiz #1)
- Fallacy Fridays: Ad Hominem, Tu Quoque
- This I Believe drafting

### Thursday, Sept 10
- Grammar: Internal Punctuation Review 1.1
- This I Believe draft launch — steps in the Google Doc:
  > 1. Graham vs. Didion — which speaks to you more, and why?
  > 2. The Persona tie-in — honest self or curated self?
  > 3. Freewrite: what's something you actually believe, not something that sounds good on paper?
  > 4. Draft (500–600 words): consider your purpose and your audience. Point to something from Didion or Graham that shapes your belief.
- AI-ometer level announced: Level 9 (brainstorm with AI, do the rest yourself — disclose any AI-assisted brainstorming in your essay)
- Exit: added your name + a one-sentence answer to "Why write?" to a shared Google Doc

### Wednesday, Sept 9
- aReading
- This I Believe worksheet (Graham/Didion crossover reading task)
- Persona exit ticket

### Tuesday, Sept 8
- Book of Alchemy essay: "Alchemy" by Pico Iyer
- Grammar: Dashes review
- Synthesis activity: one idea each from Didion, Graham, and Iyer, plus advice for seniors writing college essays

</details>

<details markdown="1">
<summary><strong>Week 2 (Aug 31–Sept 4)</strong></summary>

### Friday, Sept 4
- Finished reading Didion's "On Keeping a Notebook"
- Worked example: Didion/Graham quote pair + contrast table
- Short response (Google Form):
  > Graham argues that writing exists to help us discover what we actually think. Using the Didion statement you selected, does she write for that same reason, or for something different? Quote your Didion evidence directly, then explain specifically what in her wording supports your answer.

### Thursday, Sept 3
- Gallery walk (Rhetorical Triangle posters)
- Grammar: Semicolons and Colons
- Started reading Didion's "On Keeping a Notebook"

### Wednesday, Sept 2
- Rhetorical Triangle lesson (Ethos/Pathos/Logos) using Bush's Ground Zero remarks
- Started a group poster activity (definitions + examples of each appeal)

### Tuesday, Sept 1
- Grammar: Internal Punctuation Review
- SPACECAT analysis of Graham's essay, in groups

### Monday, Aug 31
- Read Paul Graham's "Putting Ideas into Words"
- Class discussion

</details>

<details markdown="1">
<summary><strong>Week 1 (Aug 24–28)</strong></summary>

### Friday, Aug 28
- Grammar diagnostic (baseline check, not graded on accuracy)
- Pitch document work time

### Thursday, Aug 27
- Finished Story Maker brainstorming steps
- Started the pitch document

### Wednesday, Aug 26
- Finished the news story analysis
- Story Maker: Generate Ideas step

### Tuesday, Aug 25
- "What is Newsworthy?" vocabulary
- Intro to finding sources; "Analyze a News Article" activity

### Monday, Aug 24
- Jenga get-to-know-you questions
- Note card: name, a Jenga answer, a possible story idea, a course skill to work on

</details>
